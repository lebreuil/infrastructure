...
Optional environment variables:
    BAO_NAMESPACE=platform
    SECRET_MOUNT=secret
    SECRET_KEY=argocd-github-app
    DEBUG=0
...
BAO_NAMESPACE = os.environ.get("BAO_NAMESPACE", "platform")
SECRET_MOUNT = os.environ.get("SECRET_MOUNT", "secret")
SECRET_KEY = os.environ.get("SECRET_KEY", "argocd-github-app")
...
def api_request(method, path, **kwargs):
    """
    Perform an OpenBao API request.
...
def secret_api_path():
    """
    Return the KV v2 API path.

    Example:
        secret/data/argocd-github-app
    """
...
def read_secret():
    ...
    Returns:
        dict on success
        None if the secret does not exist
    """
...
def initialise():
    ...
    if existing is not None:
        ...
        warn(
            "Secrets already exist — keys: "
            + ", ".join(existing_keys)
        )

        confirm = input(
            "Overwrite existing secrets? [y/N] "
        ).strip().lower()
    ...
    # Generate automatically generated secrets.
    secret_data = {
        key: generator()
    ...
    try:
        ...
        # Best-effort cleanup of local references.
        secret_data.clear()
